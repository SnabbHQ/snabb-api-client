[![Build Status](https://travis-ci.org/mjk/snabb-rush.svg?branch=master)](https://travis-ci.org/mjk/snabb-rush)

# snabb rush sdk

Node.js SDK for the SnabbRUSH API. Make deliveries without ever leaving your CLI.

## Prerequisites

You'll need a client ID and a client secret from Snabb. Additionally, you'll want to test in the SnabbRUSH sandbox before you start using the API to move couriers around the world.

 1. Register at [https://developer.snabb.com](http://developer.snabb.com)
 2. Create a new app
 3. Enable the delivery sandbox
 4. Copy your client ID and secret 
 5. `npm install snabb-rush`
...
Before making API calls, you must initialize the SDK with your app's ID and secret.
    ...
    const SnabbRUSH = require('snabb-rush')
    const SnabbRUSHClient = SnabbRUSH.createClient({
...
configured in a plain old JavaScript object:
	  ...
	  var delivery = SnabbRUSHClient.createDelivery({
          item: {
              title: 'Chocolate bar',
              quantity: 1,
              is_fragile: true
...
Snabb requests that we only check status every 30 seconds. If you're trying to keep up with your courier on a map, that's a slow update cycle--by default we only fire status update events on the same schedule. 
 ...
 * **admin_ended**: An administrator manually terminated this delivery from the SnabbRUSH console.

### Delivery ETAs

Estimated Time of Arrival (ETA) is expressed as an integer number of minutes from now. SnabbRUSH provides two ETAs: `dropoff_eta` (how long until the dropoff is projected to occur), and `pickup_eta` (how long until the courier will be picking up the item). These estimates are updated automatically throughout the delivery process; whenever that happens, these events are fired.
...
### Delivery
The core of the SnabbRUSH SDK, a Delivery is required to get a quote and dispatch a courier.
...
Requests a quote from the SnabbRUSH API. Returns a [promise](https://promisesaplus.com) for an array of `Quote` objects. 

Even if you're completely price-insensitive, SnabbRUSH requires a quote before initiating delivery. Thus, this is always the first step to calling a courier.
...
Throws an error if the SnabbRUSH API won't confirm your delivery.
...
Example: 
    ...
    var SnabbRUSH = require('snabb-rush');
	var loc = new SnabbRUSH.Location({
		address: '64 Seabring St',
		city: 'Brooklyn',
		state: 'NY',
		postal_code: '11201',
